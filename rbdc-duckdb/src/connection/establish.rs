...
impl Drop for DuckDbConnection {
    fn drop(&mut self) {
        // Notify the worker thread to shut down (ignore result since we cannot await in Drop)
        // Must use try_send! AsyncTx::send returns a SendFuture that will never send the message if not polled
